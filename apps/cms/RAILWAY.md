...
- **`Could not load the "sharp" module using the linuxmusl-x64 runtime`:** `@strapi/upload` depends on **sharp** with platform-specific **`@img/*`** optional packages; npm workspaces can omit them on Alpine. This repo adds **`sharp@0.33.5`** on **`apps/cms`** (same line as Strapi’s upload plugin) and root **`optionalDependencies`** for **`@img/sharp-linuxmusl-x64`** / **`@img/sharp-libvips-linuxmusl-x64`** so Strapi Cloud can resolve native libs. See [sharp install](https://sharp.pixelplumbing.com/install).
- **`Cannot find module @rollup/rollup-linux-x64-musl` (Strapi Cloud / Alpine):** Rollup’s native addon is an **optional dependency**; npm + workspaces can skip it ([npm/cli#4828](https://github.com/npm/cli/issues/4828)). The monorepo **root** [`package.json`](../package.json) lists **`@rollup/rollup-linux-x64-musl`** and **`@rollup/rollup-linux-x64-gnu`** under `optionalDependencies` (pinned to the same Rollup minor as the lockfile) so `npm ci` on Linux installs the right binary.
